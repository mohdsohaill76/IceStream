...
- **Stateful Deduplication**: Uses Flink Keyed `ValueState` with a 24-hour State TTL to drop duplicate `transaction_id` records.
...
- **Automated Testing Suite (`tests/`)**: Full unit coverage with `pytest` for parsing, validation, watermarking, state deduplication, and DLQ serialization.
...
├── src/
│   ├── __init__.py
│   └── transforms.py          # JSON parsing, validation, watermarking & deduplication logic
├── tests/
│   ├── __init__.py
